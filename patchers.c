...
#define pointer(p) (__pointer[0] = (uint32_t)p & 0xff, __pointer[1] = ((uint32_t)p / 0x100) & 0xff, __pointer[2] = ((uint32_t)p / 0x10000) & 0xff, __pointer[3] = ((uint32_t)p / 0x1000000) & 0xff, _pointer)
    ...
    *(uint32_t *)last_good_bl = bswap32(0x4ff00001);
    ...
    if (*(uint32_t *)NOPstop == bswap32(0x4ff0ff30))
    { //mov.w      r0, #0xffffffff
...
int patch_kaslr(struct iboot_img *iboot_in)
{
    ...
    {
        printf("%s: Entering...\n", __FUNCTION__);
        void *kaslr_search = find_kaslr(iboot_in, os_vers);
        if (!kaslr_search)
        {
            printf("%s: Unable to find MOV.W!\n", __FUNCTION__);
            return 0;
        }
        void *bne = kaslr_search - 0x24;
        printf("%s: KASLR BNE should be at %p\n", __FUNCTION__, (GET_IBOOT_FILE_OFFSET(iboot_in, bne)));
    ...
    else if (os_vers == 7)
    {
        ...
        void *beq = kaslr_search - 0x15;
        printf("%s: KASLR BEQ should be at %p\n", __FUNCTION__, (GET_IBOOT_FILE_OFFSET(iboot_in, beq)));
        if (*(unsigned char *)beq == 0xd0) {
            printf("%s: Successfully found D0 Hex at %p\n", __FUNCTION__, (GET_IBOOT_FILE_OFFSET(iboot_in, beq)));
        } else {
            return 0;
        }
    ...
    else if (os_vers == 8)
    {
        ...
        *(unsigned char *)beq = 0xe0; //Change D0 to E0 resulting in BEQ -> B.
        ...
        if (!kaslr_search)
        {
            printf("%s: Unable to find MOV.WEQ!\n", __FUNCTION__);
            return 0;
        }
        void *it = kaslr_search - 0x2;
        printf("%s: KASLR IT should be at %p\n", __FUNCTION__, (GET_IBOOT_FILE_OFFSET(iboot_in, it)));
        printf("%s: Patching KASLR IT at %p...\n", __FUNCTION__, GET_IBOOT_FILE_OFFSET(iboot_in, it));
